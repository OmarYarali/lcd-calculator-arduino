...
// Handling Arithmetic ops(+, -, *, /, ^)
  ...
  op = Serial.read();
  Serial.flush(); //// Clears the buffer after each input
  lcd.print(op);
  delay(1000);
  lcd.clear();
...
// Operator checking( If the user enters an invalid character, the program will do nothing).
  ...
  answer = firstNum * secondNum;
      ...
      lcd.print(errorMsg);
      delay(2000);
      return; //(If the user tries to divide First Number with 0,
      // the program will print the error message,terminate and then will start to loop again).
      }
      else
  answer = firstNum / secondNum;
  ...
  lcd.clear(); //Error h
  lcd.print("Invalid Operator"); //( If the user inputs invalid operator, 
   delay(2000);                     //the program will print error message, terminate and then will start to loop again).
  return;
  }
